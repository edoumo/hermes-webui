"""Standalone Hermes Harness backend-for-frontend foundation.

The browser talks only to same-origin /api/harness routes. This module owns the
strict allowlist and forwards requests to the canonical Hermes API with a
server-side bearer. No arbitrary target, method or header can be supplied by
the browser.
"""
...
class HarnessConfigError(RuntimeError):
    """Harness configuration is incomplete or unsafe."""
...
_ROUTES: tuple[tuple[str, re.Pattern[str], str], ...] = (
    ...
    (
        "GET",
        re.compile(rf"^/sessions/(?P<session_id>{_SAFE_ID})/workers/(?P<worker_id>{_SAFE_ID})$"),
        "/api/sessions/{session_id}/workers/{worker_id}",
    ),
    (
        "GET",
        re.compile(rf"^/sessions/(?P<session_id>{_SAFE_ID})/workers/(?P<worker_id>{_SAFE_ID})/messages$"),
    ...
    (
        "POST",
        re.compile(rf"^/sessions/(?P<session_id>{_SAFE_ID})/workers/(?P<worker_id>{_SAFE_ID})/run$"),
        "/api/sessions/{session_id}/workers/{worker_id}/run",
    ),
    (
        "GET",
        re.compile(rf"^/sessions/(?P<session_id>{_SAFE_ID})/workers/(?P<worker_id>{_SAFE_ID})/activations$"),
    ...
    (
        "POST",
        re.compile(rf"^/sessions/(?P<session_id>{_SAFE_ID})/worker-tasks/(?P<task_id>{_SAFE_ID})/status$"),
        "/api/sessions/{session_id}/worker-tasks/{task_id}/status",
    ),
    (
        "POST",
        re.compile(rf"^/sessions/(?P<session_id>{_SAFE_ID})/worker-tasks/(?P<task_id>{_SAFE_ID})/dependencies$"),
...
def harness_enabled(environ: Optional[dict[str, str]] = None) -> bool:
    ...
    if "HERMES_HARNESS_ENABLED" in env:
        return _flag(env.get("HERMES_HARNESS_ENABLED", ""))
    if "HERMES_WEBUI_HARNESS_UI" in env:
        return _flag(env.get("HERMES_WEBUI_HARNESS_UI", ""))
    return True
...
def _gateway_base_url(environ: Optional[dict[str, str]] = None) -> str:
    env = os.environ if environ is None else environ
    raw = str(
        env.get("HERMES_HARNESS_GATEWAY_BASE_URL")
        or env.get("HERMES_WEBUI_GATEWAY_BASE_URL")
    ...
    if host not in {"127.0.0.1", "localhost", "::1"} and not _flag(env.get("HERMES_HARNESS_ALLOW_REMOTE_API", "")):
        raise HarnessConfigError("Remote Hermes API requires HERMES_HARNESS_ALLOW_REMOTE_API=1")
    return raw.rstrip("/")
...
def _gateway_api_key(environ: Optional[dict[str, str]] = None) -> str:
    env = os.environ if environ is None else environ
    value = str(
        env.get("HERMES_HARNESS_GATEWAY_API_KEY")
        or env.get("HERMES_WEBUI_GATEWAY_API_KEY")
        or ""
    ).strip()
    if not value:
        raise HarnessConfigError("Hermes API key is required")
    if "\r" in value or "\n" in value:
...
def _upstream_request(handler, parsed, *, method: str, upstream_path: str, environ: Optional[dict[str, str]] = None):
    query = _safe_query(parsed.query) if method == "GET" else ""
    target = f"{_gateway_base_url(environ)}{upstream_path}"
    if query:
...
def _proxy_json(handler, parsed, *, method: str, upstream_path: str) -> bool:
    try:
        request = _upstream_request(handler, parsed, method=method, upstream_path=upstream_path)
        try:
...
def _proxy_sse(handler, parsed, *, upstream_path: str) -> bool:
    try:
        request = urllib.request.Request(
            f"{_gateway_base_url()}{upstream_path}",
            headers={
        ...
        j(handler, {"error": "Hermes API event stream is unavailable", "code": "harness_upstream_unavailable"}, status=502)
        return True
