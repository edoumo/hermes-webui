...
def test_session_load_clears_stale_stream_before_response():
    # The normalized page path may supply ``s`` without materializing the full
    # transcript. The legacy fallback remains the load boundary for this guard.
    load_pos = ROUTES_SRC.index("else get_session(sid, metadata_only=(not load_messages))")
